...
<template >
  <div>
    <div class="tasks">
      ...
      <button class="taskButton" @click="createTask()"> Create Task </button>
      <ul>
        <li key="index" class="task" v -for="(task, index) in tasks" :>
          <p class="text">
            {{ task.name }}
          </p>
          <p class="text button" @click="toggleComplete(task)"> { { task.completed ? 'completed' : 'not completed' } } </p>
          <p class="text button delete" @click="deleteTask(task)"> Delete task </p>
        </li>
      </ul>
    </div>
  </div>
...
.input {
  ...
  font-size: 18px;
  border: none;
  border-bottom: 2px solid #2196 F3;
  outline: none;
}
.button {
  cursor: pointer;
