...
<template>
  <div>
    <v-alert :value="true" class="ma-5" type="info">
      <alert info>
    ...
    <v-form ref="form" v-model="valid" class="ma-5" lazy-validation netlify>
      <v-stepper v-model="e13" vertical elevation-0>
        ...
        <v-stepper-content step="1">
          ...
          <v-container fluid>
            <v-layout row>
              ...
              <v-flex xs8>
                <v-text-field v-model="name" :rules="nameRules" label="John Doe" required />
              </v-flex>
            </v-layout>
            <v-layout row>
              <v-flex xs4>
              ...
              <v-flex xs8>
                <v-text-field v-model="phone" :rules="phoneRules" label="555-555-5555" required />
              </v-flex>
            </v-layout>
            <v-layout row>
              <v-flex xs4>
              ...
              <v-flex xs8>
                <v-text-field v-model="email" :rules="emailRules" label="johndoe@gmail.com" required />
              </v-flex>
            </v-layout>
            <v-layout row>
              <v-flex xs4>
        ...
        <v-stepper-content step="2">
          <v-text-field ref="address" :rules="[
            ...
            ]" v-model="address" label="Address Line" placeholder="Snowy Rock Pl" counter="25" required />
          <v-text-field ref="city" :rules="[() => !!city || 'This field is required']" v-model="city" label="City" placeholder="El Paso" required />
          <v-text-field ref="state" v-model="state" :rules="[() => !!state || 'This field is required']" label="State/Province/Region" required placeholder="TX" />
          <v-text-field ref="zip" :rules="[() => !!zip || 'This field is required']" v-model="zip" label="ZIP / Postal Code" required placeholder="79938" />
        ...
        <v-stepper-content step="3">
          ...
          <v-stepper v-model="e1">
            <div v-for="n in historySteps" :key="n.id">
              ...
              <v-stepper-content :step="n">
                <v-layout row wrap>
                  <v-flex xs11 sm5>
                    <v-text-field :v-model="`position-${n}`" box label="Place of employment" />
                  </v-flex>
                  <v-spacer/>
                  <v-flex xs11 sm5>
                    <v-text-field :v-model="`position-${n}`" box label="Position" />
                  </v-flex>
                </v-layout>
                <v-layout row wrap>
                  <v-flex xs11 sm5>
                    <v-menu ref="menu" :close-on-content-click="false" v-model="menu" :nudge-right="40" :return-value.sync="date" lazy transition="scale-transition" offset-y full-width min-width="290px">
                      <v-text-field slot="activator" :v-model="`startDate-${n}`" label="Start Date" prepend-icon="event" readonly />
                  ...
                  <v-flex xs11 sm5>
                    <v-menu ref="menu2" :close-on-content-click="false" v-model="menu2" :nudge-right="40" :return-value.sync="date" lazy transition="scale-transition" offset-y full-width min-width="290px">
                      <v-text-field slot="activator" :v-model="`endDate-${n}`" label="End Date" prepend-icon="event" readonly />
                      <v-date-picker v-model="date" no-title scrollable>
                        <v-spacer/>
                        <v-btn flat color="primary" @click="menu = false">Cancel</v-btn>
                        <v-btn flat color="primary" @click="$refs.menu.save(date)">OK</v-btn>
                ...
                <v-flex xs12>
                  <v-text-field :v-model="`${n}-company`" label="Reason for leaving" box multi-line />
                </v-flex>

                <!-- <v-card color="grey lighten-1" class="mb-5" height="200px"></v-card> -->
                <v-btn flat small outline color="primary" @click="nextStep(n)">Add another job/>
                  <v-btn flat small outline>Remove job history/>
                  </v-btn>
                </v-btn>
              </v-stepper-content>
              <!-- </v-stepper-items> -->
        ...
        <v-stepper-content step="4">
          ...
          <v-container fluid>
            <v-layout v-for="d in 6" :key="d.id" row wrap>
              <v-flex xs6>
                <v-subheader>{{ weekDays[d] }}</v-subheader>
              </v-flex>
              <v-flex xs6>
                <v-select :items="times" :v-model="`${d}-time`" label="Select" single-line />
              </v-flex>
            </v-layout>
          </v-container>
          <!-- </v-card> -->
        ...
        <v-stepper-content step="5">
          ...
          <v-checkbox v-model="checkbox" :rules="[v => !!v || 'You must agree to continue!']" label="I agree to allow Andrews Flooring to run a background check based on the information I have provided with this application." required />
