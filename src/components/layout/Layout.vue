    ...
    <template v-if="isAuthenticated">
      ...
      <v-footer app>
        <v-toolbar flat dense color="primary" dark>
          <v-toolbar-side-icon :class="searching ? 'hidden-xs-only' : ''" class="hidden-lg-and-up" @click.stop="drawer = !drawer" />
          ...
          <div :class="{'searching--closed': !searching}" class="searching">
            <v-text-field id="search" v-model="search" append-icon="close" label="Search" hide-details single-line color="white" @blur="onBlur" />
          </div>
...
export default {
  ...
  methods: {
    ...
    beforeUpdate() {
      console.log('store.username: ' + this.$store.state.user)
